...
<template>
    ...
    <AuthenticatedLayout>
        <div class="py-8 px-8 bg-gray-50 min-h-screen">
            <div class="bg-white rounded-xl shadow-sm overflow-hidden animate-fade-in border border-gray-100">
                ...
                <div class="p-6 bg-gradient-to-br from-gray-50 to-gray-100 border-b border-gray-200">
                    <div class="flex justify-between items-center">
                        <h2 class="text-xl font-semibold text-green-900">All Trainers ({{ filteredTrainers.length }})</h2>
                        <div class="relative">
                            <input
                                v-model="searchQuery"
                                @input="updateFilter"
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="trainer in filteredTrainers" :key="trainer.id" class="hover:bg-green-50 transition duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    TR{{ String(trainer.id).padStart(3, "0") }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="h-10 w-10 rounded-full bg-green-600 flex items-center justify-center text-white font-semibold mr-3">
                                            {{ trainer.avatar }}
                                        </div>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-1">
                                            ...
                                            <span class="text-xs">{{ trainer.email }}</span>
                                        </div>
                                        <div class="flex items-center gap-1">
                                            <svg class="h-4 w-4 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                                            </svg>
                                            <span class="text-xs">{{ trainer.phone }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-1">
                                            <span class="text-xs text-gray-500">Courses:</span>
                                            <span class="font-semibold text-emerald-600">{{ trainer.activeCourses }}</span>
                                        </div>
                                        <div class="flex items-center gap-1">
                                            <span class="text-xs text-gray-500">Trainees:</span>
                                            <span class="font-semibold text-green-600">{{ trainer.totalTrainees }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        :class="{
                                            'bg-green-100 text-green-800': trainer.status === 'active',
                                            'bg-gray-100 text-gray-800': trainer.status === 'inactive',
                                        }"
                                        class="inline-flex px-2 py-1 text-xs font-semibold rounded-full"
                                    >
                                        {{ trainer.status?.charAt(0).toUpperCase() + trainer.status?.slice(1) || "Active" }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm font-medium">
                                    <div class="flex items-center gap-2">
                                        <button @click="viewTrainer(trainer)" class="text-green-700 hover:text-green-900 p-1 rounded" title="View">
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                            </svg>
                                        </button>
                                        <button @click="editTrainer(trainer)" class="text-green-700 hover:text-green-900 p-1 rounded" title="Edit">
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </button>
                                        <button @click="deleteTrainer(trainer)" :disabled="processing" class="text-red-600 hover:text-red-800 p-1 rounded" title="Delete">
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Empty State -->
                    <div v-if="filteredTrainers.length === 0" class="p-8 text-center bg-gradient-to-br from-white to-green-50">
                        <svg class="mx-auto h-12 w-12 text-green-300 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.196-2.196m0 0A5.002 5.002 0 0019 15m-3 4h-3a4 4 0 01-4-4v-1m6 5H9a2 2 0 01-2-2v-1" />
                        </svg>
